...
class ImageProcessor {
    ...
    //     });
    // }
    analyzeImageBrightnessContrast(file) {
            ...
            reader.onload = (event) => {
                ...
                img.onload = () => {
                    ...
                    for (let i = 0; i < data.length; i += 4) {
                        ...
                        const avg = (r + g + b) / 3;
                        brightnessSum += avg;
                        contrastSum += Math.abs(avg - 128);
                    }
    
                    const brightness = brightnessSum / totalPixels;
                    const contrast = contrastSum / totalPixels;
    ...
    processImageWithEnhancement(file, enhancementOptions) {
        return __awaiter(this, void 0, void 0, function* () {
            let analyzedBrightness = 0;
            let analyzedContrast = 0;
            let analyzedSaturation = 1;
    
            // Read image data to analyze brightness & contrast
            const analysisResult = yield this.analyzeImageBrightnessContrast(file);
            analyzedBrightness = analysisResult.brightness;
            analyzedContrast = analysisResult.contrast;
    
            // Auto-adjust brightness, contrast, and saturation if not provided
            const options = Object.assign(Object.assign({}, this.defaultEnhancementOptions), enhancementOptions);
    
            if (options.brightness === undefined) {
                options.brightness = analyzedBrightness < 100 ? 10 : -30; // Adjust for dark or overexposed images
            }
            if (options.contrast === undefined) {
                options.contrast = analyzedContrast < 100 ? 40 : 20; // Increase contrast for dull images
            }
            if (options.saturation === undefined) {
                options.saturation = analyzedBrightness < 100 ? 1.2 : 0.9; // Slightly boost saturation in dark images
            }
